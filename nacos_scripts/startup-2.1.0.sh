...
if [[ "$JAVA_OPT_EXT_FIX" == "" ]]; then
  CMD="nohup $JAVA ${JAVA_OPT} nacos.nacos >> ${BASE_DIR}/logs/start.out 2>&1 &"
  su -s /bin/bash -c "${CMD}" nacos
else
  CMD="nohup $JAVA $JAVA_OPT_EXT_FIX ${JAVA_OPT} nacos.nacos >> ${BASE_DIR}/logs/start.out 2>&1 &"
  su -s /bin/bash -c "${CMD}" nacos
fi
